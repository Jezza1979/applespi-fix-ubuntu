...
#ifndef APPLE_HID_DRV_INFO_DEFINED
#define APPLE_HID_DRV_INFO_DEFINED
struct apple_hid_drv_info {
    struct list_head entry;
    void *driver;  // Adjust this field’s type if needed
};
#endif
...
static void appleib_detach_and_free_hid_driver(struct appleib_device *ib_dev, struct apple_hid_drv_info *hid);
...
void appleib_detach_and_free_hid_driver(struct appleib_device *ib_dev, struct apple_hid_drv_info *hid)
{
    list_del_rcu(&hid->entry);
    /* Detach devices using the driver pointer */
    appleib_detach_devices(ib_dev, hid->driver);
    kfree(hid);
}
...
int appleib_unregister_hid_driver(struct appleib_device *ib_dev,
				  struct hid_driver *driver)
{
	struct apple_hid_drv_info *drv_info;
...
/* change from int to void since int new kernal 6.8 expects void */
static void appleib_remove(struct acpi_device *acpi)
{
	...
	/*  Remove return since now void, not int*/
	// return 0;
}
